@extends('master')
@section('content')
<h1 class="text-white px-6 text-left text-2xl font-bold my-5">Edit Movie</h1>
<form action="/admin/{{$movie->id}}" method="post" enctype="multipart/form-data" class="px-6 space-y-6">
    @csrf
    @method('put')
    ...
    <div>
        ...
        <input type="text" name="name" placeholder="Masukkan judul movie" value="{{ $movie->name }}"
    ...
    <div>
        ...
        <input type="text" name="studio" placeholder="Masukkan studio umum" value="{{ $movie->studio }}"
            class="w-full py-2.5 px-3 rounded-sm border border-blue-500 bg-gray-800 bg-opacity-65 text-white placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-white">
        @error('studio')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
    ...
    <div>
        ...
        <input type="text" name="link" placeholder="Masukkan link YouTube" value="{{ $movie->link }}"
            class="w-full py-2.5 px-3 rounded-sm border border-blue-500 bg-gray-800 bg-opacity-65 text-white placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-white">
        @error('link')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            ...
            @error('image')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
            <div id="image-preview" class="mt-4 relative">
                @if($movie->image)
                    <img src="{{ asset('storage/' . $movie->image) }}" alt="Thumbnail Movie"
                        class="rounded-md border border-blue-500 shadow-md shadow-blue-500 absolute bottom-10 right-10 w-40 md:w-60 lg:w-80">
                @endif
            </div>
    ...
    <div>
        ...
        <input type="text" name="tags" placeholder="Action | Adventure | Fiction" value="{{ $movie->tags }}"
            class="w-full py-2.5 px-3 rounded-sm border border-blue-500 bg-gray-800 bg-opacity-65 text-white placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-white">
    </div>
    ...
    <div class="flex justify-between">
        <a href="./"
            class="flex items-center font-semibold text-white gap-x-1 bg-red-500 hover:bg-red-600 px-4 py-2 rounded">
