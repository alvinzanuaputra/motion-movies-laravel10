        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
            @foreach ($movies as $i)
                ...
                <div class="bg-gray-800 text-white shadow-md rounded-lg overflow-hidden">
                    ...
                    <div class="p-4">
                        <h5 class="text-lg font-semibold">{{ \Illuminate\Support\Str::words($i->name, 4, '...') }}</h5>
                        <p class="text-gray-400">{{ $i->studio }}</p>
                        <a href="/movie/{{ $i->id }}"
                            class="flex items-center mt-4 mr-16 md:mr-20 lg:mr-44 bg-blue-600 text-white py-1 gap-x-1 px-4 rounded hover:bg-blue-700 hover:scale-95 duration-500 transition-all">
                            Lihat
                        </a>
                    </div>
                </div>
    ...
    @else
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
            @foreach ($newMovies as $movie)
                ...
                @endif
                <div
                    class="bg-gray-800 text-white shadow-md rounded-lg border hover:border-white overflow-hidden hover:scale-95 shadow-black hover:shadow-blue-500 duration-500 transition-all">
                    ...
                    <div class="p-4">
                        ...
                        <a href="/movie/{{ $movie->id }}"
                            class="mt-4 inline-block bg-blue-600 text-white py-1 gap-x-1 px-4 rounded hover:bg-blue-700 hover:scale-95 duration-500 transition-all">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
                @foreach ($genres[$i]->movie as $movie)
                    ...
                    <div
                        class="bg-gray-800 text-white shadow-md rounded-lg border hover:border-white overflow-hidden hover:scale-95 shadow-black hover:shadow-blue-500 duration-500 transition-all">
                        <img src="{{ asset('storage/' . $movie->image) }}" alt="{{ asset('images/alt.png') }}"
                            class="w-full h-84 object-cover object-center">
                        <div class="p-4 mt-2 rounded-lg bg-gray-800">
                            <h5 class="text-lg font-semibold">{{ \Illuminate\Support\Str::words($movie->name, 2, '...') }}</h5>
                            <p class="text-gray-400">{{ $movie->studio }}</p>
                            <a href="/movie/{{ $movie->id }}"
                                class="mt-4 inline-block bg-blue-600 text-white py-1 gap-x-1 px-4 rounded hover:bg-blue-700 hover:scale-95 duration-500 transition-all">
                                <div class="flex items-center gap-x-2">
